    ...
    func compilerAction() -> Bool {
        ...
        if !input.isEmpty && !output.isEmpty {
            ...
            if checkError(result) {
                alert = result
                showAlert("Compiler gooide fout",result)
                result.removeAll()
                return false
            } else {
        ...
        VStack {
            VStack {
                HStack {
                    Text("Broncode: ")
                    ZStack {
                        TextField("Invoerbestand",text: $input)
                        ...
                        HStack {
                            Spacer()
                            Button("kiezen...") {
                                input = chooseFile("Kies Invoerbestand...",false)
                            }
                            .buttonStyle(RoundedRectangleButtonStyle())
                            .padding(.trailing,4)
                        }
                ...
                HStack {
                    Text("Uitvoer bestand: ")
                    ZStack {
                        TextField("Gecompileerd uitvoerbaar bestand",text: $output)
                            .font(.custom("Menlo",size:12))
                            .padding(.leading,7)
                            .textFieldStyle(PlainTextFieldStyle())
                            .frame(height:28)
                        ...
                        HStack {
                            Spacer()
                            Button("kiezen...") {
                                output = chooseFile("Kies Output Directory...",true)
                            }
                            .buttonStyle(RoundedRectangleButtonStyle())
                            .padding(.trailing,4)
                        }
                ...
                HStack {
                    Toggle(isOn:$isFrameworkCompile) {
                        Text("Gebruik framework: ")
                    }
                    TextField(isFrameworkCompile ? "Kader1, Kader2...":"Framework Uitgeschakeld...",text:$framework)
                        .font(.custom("Menlo",size:12))
                        .padding(.leading,5)
                        .textFieldStyle(PlainTextFieldStyle())
                        .frame(height:22)
                ...
                HStack {
                    ...
                    Text(alert)
                        ...
                        .font(.custom("Menlo",size:12))
                    Spacer()
                    Button("Uitvoeren en fouten opsporen") {
                        if compilerAction() {
...
struct debuggerSuggestionView_Previews:PreviewProvider {
    static var previews:some View {
        @State var debuggerSuggestion = "Analyseren heeft niets..."
